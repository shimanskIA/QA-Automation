...
namespace MSTestsForTask4
{
    ...
    public class XmlInteractionMethodsTests
    {
        ...
        private static Action<List<Vehicle>, string> listComparePositive = (x, y) => Assert.IsTrue(x.SequenceEqual(Helper.ReadFromXmlFile<Vehicle>(y)));
        private static Action<List<Vehicle>, string> listCompareNegative = (x, y) => Assert.IsFalse(x.SequenceEqual(Helper.ReadFromXmlFile<Vehicle>(y)));
        ...
        {
            Helper.WriteToXmlFile(objects, fileName);
