...
namespace MSTestsForTask8
{
    ...
    public class CarParkMethodsTests
    {
        ...
        private static readonly CarPark _carParkBeforeUpdate = new CarPark(new List<Vehicle>() { _auto1, _bus1, _lorry1, _scooter1});
        private static readonly CarPark _carParkAfterUpdate = new CarPark(new List<Vehicle>() { _auto2, _bus1, _lorry1, _scooter1});
        private static readonly CarPark _carParkBeforeRemove = new CarPark(new List<Vehicle>() { _auto1, _bus1, _lorry1, _scooter1 });
        private static readonly CarPark _carParkAfterRemove = new CarPark(new List<Vehicle>() { _bus1, _lorry1, _scooter1 });
        ...
        private static Action<CarPark, CarPark> carParkComparePositive = (x, y) => Assert.IsTrue(x.Vehicles.SequenceEqual(y.Vehicles));
        private static Action<CarPark, CarPark> carParkCompareNegative = (x, y) => Assert.IsFalse(x.Vehicles.SequenceEqual(y.Vehicles));
        ...
        [TestMethod]
        [DynamicData(nameof(GetDataUpdateMethodTest), DynamicDataSourceType.Method)]

        public void UpdateMethodTest(CarPark testCarParkBefore, CarPark testCarParkAfter, int id, Vehicle vehicle, Action<CarPark, CarPark> assert)
        {
            testCarParkBefore.Update(id, vehicle);
            assert(testCarParkBefore, testCarParkAfter);
        }

        public static IEnumerable<object[]> GetDataUpdateMethodTest()
        {
            yield return new object[] { _carParkBeforeUpdate, _carParkAfterUpdate, 0, _auto2, carParkComparePositive };
            yield return new object[] { _carParkBeforeUpdate, _carParkAfterUpdate, 2, _auto2, carParkCompareNegative };
        }
    }
}
